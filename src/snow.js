...
export default window.snowStorm = new (function () {
  ...
  Object.keys(defaults).forEach(function (key) {
    if (key in window) {
      console.warn(
        "[SnowStorm] You are using a deprecated configuration key `" +
          key +
          "`. Please see <https://dev.fandom.com/wiki/SnowStorm> for supported parameter."
      );
  ...
  this.mouseMove = function (e) {
    ...
    if (x < screenX2) {
      storm.windOffset =
        -storm.windMultiplier + (x / screenX2) * storm.windMultiplier;
    } else {
      x -= screenX2;
  ...
  this.createSnow = function (limit, allowInactive) {
    ...
    for (i = 0; i < limit; i++) {
      storm.flakes[storm.flakes.length] = new storm.SnowFlake(
        parseInt(rnd(storm.flakeTypes), 10)
      );
